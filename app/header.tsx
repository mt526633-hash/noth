...
export function PersistentHeader(){
  ...
  useLayoutEffect(()=>{
    ...
    if(!nav)return;
    const update=()=>{const active=nav.querySelector<HTMLAnchorElement>("a.current");setIndicator(active?{left:active.offsetLeft,width:active.offsetWidth,visible:true}:{left:0,width:0,visible:false})};
    update();
  ...
  return <header className="topbar"><Link className="logo" href="/" onClick={prepareRoute}><span className="logo-mark"><House weight="fill"/></span><span>Northline<small>House engineering</small></span></Link><nav ref={navRef} className={menu?"nav open":"nav"}>{navigation.map(item=><Link key={item.href} className={pathname===item.href?"current":""} href={item.href} onClick={prepareRoute}>{item.label}</Link>)}<span className={`nav-indicator${indicator.visible?" visible":""}`} style={{transform:`translate3d(${indicator.left}px,0,0)`,width:indicator.width}}/></nav><button className="button button-dark nav-button" onClick={openNotice}><span>Start a project</span><ArrowRight/></button><button className="menu" onClick={()=>setMenu(!menu)} aria-label="Toggle navigation">{menu?<X/>:<List/>}</button></header>
}
