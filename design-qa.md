...
- The active navigation state is now one persistent `72px × 34px` translucent glass capsule centered behind every label. Width and height have no transition and remain fixed; only horizontal transform, route tint, border, fill and shadow interpolate on a smooth 780ms curve. The settled Studio state was visually verified in-browser with one capsule and no duplicate underline, size animation or console error.
