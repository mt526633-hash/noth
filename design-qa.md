...
- Resting indicator width is fixed at `34px` for every route and centered below each label. The rail has been visually rebuilt as a 2px tapered light filament inside an 8px effect track, with a blurred route-color aura and white energy core. Journal → Projects sampled `1.23393×` during early extension, `0.720288×` during pre-arrival compression, and `1×` at the destination; the outer track remained `34px` throughout.
