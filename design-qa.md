...
- Resting indicator width is now fixed at `34px` for every route and centered below each label. Studio → Projects measured `34px` before, during and after travel at the outer rail; only the inner light streak elastically reached `2.14481×` during motion before returning to an unscaled `34px` resting state.
